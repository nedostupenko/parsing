...
for doc in total_vacancies:
    search_res = vacancies.find_one(doc)
    if not search_res:
...
salary_value = 100000
for doc in db.vacancies.find({'$or':[{'salary_up': {'$gte':salary_value}}, {'salary_to': {'$gte':salary_value}}]}):
    pprint(doc)